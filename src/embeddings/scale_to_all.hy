;; Scale to all French verbs
;; Run with: poetry run hy src/embeddings/scale_to_all.hy
...
(defn main []
  (print "🔎 Scaling French Verb Embeddings to ALL verbs")
  ...
  ;; Load all verbs (no limit)
  (setv verbs (load-verbs full-verbs-path))
  ...
  ;; Save results
  (setv scale-dir (os.path.join output-dir "scaling_all"))
  (os.makedirs scale-dir :exist-ok True)
  
  ;; Save embeddings
  (setv embeddings-file (os.path.join scale-dir "embeddings_all.json"))
  ...
  ;; Save stats  
  (setv stats-file (os.path.join scale-dir "stats_all.json"))
